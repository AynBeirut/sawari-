...
let itemsHtml = items.map(item => {
  let subHtml = item.subtitle ? \n            <p class="fp-subtitle-type">\</p> : '';
  let statsHtml = item.stats.map(s => <p>\</p>).join('\n              ');
  let kpHtml = item.kp.map(k => <p>\</p>).join('\n              ');
...
;
  let imgBlock = 
        <div class="fp-image">
          ...
          <img src="design/UNIT RENDER/UNIT RENDER/\" alt="\" />
        </div>
;

  return 
      <article class="fp-item row-bg-\">
\
      </article>
;
}).join('');
